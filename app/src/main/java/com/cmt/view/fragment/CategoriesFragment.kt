...
import com.cmt.databinding.FragmentCategoriesBinding
...
class CategoriesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        setData()
        binding.viewModel?.setCategories(binding.recyclerView,pageListCont.toString())
        binding.viewModel?.categoriesData?.observe(requireActivity()) {
            if(it != null){
                totalPagesFound = it.total_pages ?: 0
                val listData = (it.results as MutableList<*>).filterIsInstance<HomeBannerModel>().toMutableList()
    ...
    private fun setData() {
        categoriesListAdapter = CategoriesAdapter(requireActivity(), categoriesList)
