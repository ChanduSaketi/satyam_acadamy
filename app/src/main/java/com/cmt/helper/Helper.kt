...
//    params[IConstants.Params.access_token] = Settings.Secure.ANDROID_ID
//    params[IConstants.Params.token] = Settings.Secure.getString(context.getContentResolver(), Settings.Secure.ANDROID_ID)
//    params[IConstants.Params.token] = AppPreferences().getPushNotificationToken(context) ?: ""
    return params
}
...
fun View.setSnackBar(context: Context, message: String?) {
    if (message != null) {
        val snackBar = Snackbar.make(this, message, Snackbar.LENGTH_LONG)
        val view = snackBar.view
        val textView: TextView = view.findViewById(R.id.snackbar_text)
        textView.typeface = ResourcesCompat.getFont(context, R.font.regular)
        textView.maxLines = 5
        textView.ellipsize = TextUtils.TruncateAt.END
        snackBar.show()
    }
